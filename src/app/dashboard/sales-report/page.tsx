...
export default function SalesReport() {
  ...
  const [transactions, setTransactions] = useState<ProcessedOrder[]>([]);
  ...
  const fetchSummaryStats = useCallback(async () => {
    try {
      ...
      let query = salesDb
        .from('orders')
        .select('order_id, net_total', { count: 'exact' })
  ...
  }, [dateRange, selectedBranch, selectedTerminal, pagination.currentPage, pagination.itemsPerPage, fetchSummaryStats]);
  ...
  return (
    <div className="min-h-screen bg-sky-100">
      ...
      <div className="mb-6 sm:mb-8 p-4">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-sky-800">Sales Report</h1>
        <p className="mt-1 sm:mt-2 text-sm sm:text-base text-sky-700">Detailed transaction report with comprehensive analytics</p>
      </div>

      {/* Filters and Summary Section */}
      <div className="mb-6 sm:mb-8 bg-white rounded-lg shadow-md border border-sky-200">
        <div className="p-4 sm:p-8 space-y-6 sm:space-y-8">
          {/* Filters Section */}
          <div className="flex flex-col lg:flex-row gap-4 bg-white rounded-lg p-4 shadow-md border border-sky-200">
            {/* Date Range Selector */}
              ...
              <div className="flex-1 grid grid-cols-2 gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full border border-sky-300 bg-white rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 shadow-sm text-sky-900 transition-all duration-200 pt-3"
                  />
                </div>
                <div className="relative">
                  <div className="absolute -top-2 left-2 px-1 bg-white text-xs font-medium text-sky-600">
            ...
            {/* Vertical Divider */}
            <div className="hidden lg:block w-px bg-sky-200 mx-2"></div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-8 md:grid-cols-3">
            <div className="bg-white p-4 sm:p-8 rounded-lg border border-sky-200 shadow-md">
              <div className="flex items-center space-x-3 sm:space-x-4">
                <div className="p-2 sm:p-3 bg-sky-100 rounded-md">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-sky-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <div>
                  <div className="text-xs sm:text-sm font-medium text-sky-700">Total Sales</div>
                  <div className="text-lg sm:text-2xl font-bold text-sky-800 mt-1">{formatCurrency(summaryStats.totalSales)}</div>
                </div>
              </div>
            </div>
            <div className="bg-white p-4 sm:p-8 rounded-lg border border-sky-200 shadow-md">
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div>
                  <div className="text-xs sm:text-sm font-medium text-sky-700">Transactions</div>
                  <div className="text-lg sm:text-2xl font-bold text-sky-800 mt-1">{summaryStats.totalTransactions}</div>
                </div>
              </div>
            </div>
            <div className="bg-white p-4 sm:p-8 rounded-lg border border-sky-200 shadow-md">
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div>
                  <div className="text-xs sm:text-sm font-medium text-sky-700">Average Ticket</div>
                  <div className="text-lg sm:text-2xl font-bold text-sky-800 mt-1">{formatCurrency(summaryStats.averageTicket)}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <>
          <div className="space-y-4 sm:space-y-6">
            {transactions.map((transaction, tIndex) => (
              <div
                ...
                className="bg-white rounded-lg shadow-md border border-sky-200 overflow-hidden transition-all duration-200 hover:shadow-lg"
              >
                {/* Transaction Header */}
                <button
                  onClick={() => toggleTransaction(transaction)}
                  className="w-full px-4 sm:px-8 py-4 sm:py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white hover:bg-sky-50 transition-colors duration-200 gap-2 sm:gap-0"
                >
                  <div className="flex items-center space-x-3 sm:space-x-4 w-full sm:w-auto">
                    <div className="flex-shrink-0">
                      <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-md bg-sky-100 flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2">
                        ...
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-sky-100 text-sky-800 mt-1 sm:mt-0">
                  ...
                  <div className="flex flex-row sm:flex-col items-end sm:items-end space-x-2 sm:space-x-0 sm:space-y-1 w-full sm:w-auto justify-between sm:justify-end">
                    <div className="text-right">
                      <p className="text-base sm:text-lg font-semibold text-sky-900">
                        {formatCurrency(transaction.final_net_total)}
                      </p>
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-sky-100 text-xs sm:text-sm">
                          ...
                          {transaction.details?.map((detail, dIndex) => (
                            <React.Fragment key={detail.id ?? `${detail.order_detail_id}-${dIndex}`}>
                              ...
                              <tr className={`${detail.voided ? 'bg-red-50' : ''} transition-colors duration-150 hover:bg-sky-50`}>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-sky-900">
                                  <div className="flex items-center">
                                    <span className={`font-medium ${detail.voided ? 'text-red-700' : 'text-sky-900'}`}>{detail.menu_name}</span>
                                    {detail.voided && (
                                ...
                                <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-sky-900 text-right font-medium">{detail.item_qty}</td>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-sky-900 text-right font-medium">{formatCurrency(detail.item_qty * detail.unit_price)}</td>
                              </tr>
                              ...
                              //       </div>
                              //     </td>
                              //     <td className="px-2 sm:px-3 py-1 sm:py-2 whitespace-nowrap text-sky-700 text-right">1</td>
                              //     <td className="px-2 sm:px-3 py-1 sm:py-2 whitespace-nowrap text-sky-700 text-right">{formatCurrency(detail.service_charge)}</td>
                              //     <td className="px-2 sm:px-3 py-1 sm:py-2 whitespace-nowrap text-sky-700 text-right">{formatCurrency(detail.service_charge)}</td>
                              //   </tr>
                              // )
                              }
                        ...
                        <tfoot>
                          ...
                          {/* Additional Discounts from orders_discounts */}
                          {transaction.discounts && transaction.discounts.length > 0 && (
                            <>
                              {transaction.discounts.map((discount, index) => (
                                <tr key={index} className="bg-sky-50">
                                  <td colSpan={2} className="px-2 sm:px-3 py-2 sm:py-3 text-sm text-sky-500 font-medium">
                                    {discount.discount_name || 'Additional Discount'}
                                  </td>
                                  <td colSpan={2} className="px-2 sm:px-3 py-2 sm:py-3 text-sm text-sky-900 text-right font-semibold">
                                    -{formatCurrency(discount.subtotal_discount)}
                                  </td>
                                </tr>
                              ))}
                            </>
                          ...
                          {transaction.tax_detail && (
                            <>
                              ...
                              {transaction.tax_detail.sc_vat_deduction > 0 && (
                              <tr className="bg-sky-50/80">
          ...
          <div className="flex justify-center items-center mt-8 space-x-4">
            <button
              className="px-4 py-2 rounded-md bg-sky-100 text-sky-700 font-medium hover:bg-sky-200 disabled:bg-sky-50 disabled:text-sky-400 transition-colors duration-200"
